...
def main():
    ...
    cfgs = pickle.load(open(f"{log_path}/cfgs.pkl", "rb"))
    if isinstance(cfgs, list):
        if len(cfgs) == 0:
            raise RuntimeError(f"Loaded cfgs.pkl from '{log_path}' but it contains no configurations.")
        cfg = cfgs[0]
    else:
        cfg = cfgs
    ...
    runner = OnPolicyRunner(env, cfg, log_path, device=gs.device) # type: ignore
    runner.load(model)
